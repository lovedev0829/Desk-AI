...
export default function TranscribeTextView({
  ...
  return (
    <Box sx={{ display: 'flex', gap: 2, p: 2, ...sx }}>
            ...
            {data.segments.map((segment, index) => (
              ...
              >
                <Stack direction="row" spacing={1} sx={{ mb: 1 }}>
                  <Typography variant="caption" style={{ display: toggleTimeStamp ? "block" : "none" }} color='gray'>
      ...
      {/* Right Panel: Export and Advanced Export Options */}
      <Box sx={{ minWidth: 220, borderLeft: '1px solid #ccc', p: 2, maxHeight: 455, mt: 5 }}>
        ...
        <List disablePadding>
          <ListItem disablePadding>
            <ListItemButton onClick={handleDownloadPDF}>
              <FileThumbnail file="example.pdf" sx={{ width: "25px" }} />
              <ListItemText primary="Download PDF" />
            </ListItemButton>
          </ListItem>
          <ListItem disablePadding>
            <ListItemButton onClick={handleDownloadDOC}>
              <FileThumbnail file="example.doc" sx={{ width: "25px" }} />
